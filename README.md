...
- **PR 리뷰**: 긴 컨텍스트를 유지하면 AI 응답 품질이 떨어지기 때문에, 기능별로 브랜치와 PR을 분리하여 구현했습니다. 같은 AI가 작성한 코드는 스스로 문제를 찾기 어려울 수 있으므로 별도 AI(Claude, Codex)로 리뷰를 진행하여 N+1 쿼리, 벌크 삭제 최적화, 트랜잭션 이슈 등을 발견하고 반영
